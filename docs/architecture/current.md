...
`src/main/index.ts` 创建窗口、管理 Electron 生命周期、注册认证/订阅 IPC，并加载全局素材库、TTS 和 Agent IPC。登录 Token 只保存在 Main 进程内存，不返回给 Renderer。全局素材库索引保存在 `app.getPath('userData')/media-library/index.json`，只记录来源路径和文件元数据，不复制或修改用户的源媒体文件。
...
Renderer 负责 React 页面、总工作区、全局素材库、TTS/插件界面和视频编辑器界面。全局素材库通过 `window.api` 加载持久化索引、调用原生文件选择并按类型筛选；它不直接读取文件系统。视频编辑器当前运行在 Renderer 中，项目草稿和媒体预览仍是工作区生命周期内的临时状态；真实项目保存、加载和编辑器项目媒体 IPC 尚未实现。
...
1. `MediaLibraryView` 通过 Preload 请求 Main 打开原生多选文件框。
2. Main 仅接受视频、音频和图片扩展名，按规范化来源路径去重，并将基础元数据写入版本化 JSON 索引。
3. 页面加载或手动刷新时，Main 重新检查每个来源文件；无法访问的条目标记为 `missing`，索引条目不会被静默删除。
4. 当前全局索引尚未接入项目引用次数、标签、重新定位和未引用缓存清理，这些能力属于计划中。
