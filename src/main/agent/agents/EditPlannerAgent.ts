...
function range(value: unknown, fallback: [number, number], min: number, max: number): [number, number] {
...
function normalize(output: StrategyModelOutput): EditStrategy {
  return {
    veryFastClipSeconds: range(output.veryFastClipSeconds, DEFAULT_STRATEGY.veryFastClipSeconds, 1.2, 4),
    ...
    avoidRecentAssetCount: Math.round(Math.min(8, Math.max(1, Number(output.avoidRecentAssetCount) || 3))),
        ...
        const output = await this.runtime.model.completeJson<StrategyModelOutput>({
          ...
          user: JSON.stringify({
            ...
            story: {
              ...
              tone: story.tone,
              segments: story.segments.map(({ id, role, pace, importance }) => ({ id, role, pace, importance }))
            }
          })
        })
